...
/*//////////////*/
/*requirement switch*/
const requirementItem = document.querySelectorAll(".requirement_left_item");
const requirementRight = document.querySelector(".requirement_right_category");
const requirementButtonsReq = document.querySelector(".switch_buttons.REQ")
const requirementPrev = document.querySelector(".switch_arrowLeft.REQ");
const requirementNext = document.querySelector(".switch_arrowRight.REQ");
for (let i = 0; i < requirementItem.length; i++) {
    requirementRight.insertAdjacentHTML("beforeend", `<div class="requirement_right_item">
    <p>${document.querySelectorAll(".requirement_left_item>h2")[i].innerHTML}</p>
    </div>`)
}
for (let i = 0; i < requirementItem.length; i++) {
    requirementButtonsReq.insertAdjacentHTML("afterbegin", `<div class="switch_buttons_button REQ"></div>`)
}
const requirementList = document.querySelectorAll(".requirement_right_item");
const requirementButtons = document.querySelectorAll(".switch_buttons_button.REQ");
SetClass(requirementList, LastElement.ReqElement, "active", "add", 1)
SetClass(requirementButtons, LastElement.ReqElement, "active", "add", 1)
SwitchButtons(requirementButtons, LastElement.ReqElement, colorsBlacks);
function Changerequirement(index, LastElement) {
    requirementList[LastElement].classList.remove("active")
    requirementButtons[LastElement].classList.remove("active")
    requirementItem[LastElement].classList.remove("active")
    requirementList[index].classList.add("active")
    requirementButtons[index].classList.add("active")
    requirementItem[index].classList.add("active")
    SwitchButtons(requirementButtons, index, colorsBlacks)
}
requirementList.forEach((elem, index) => {
    elem.addEventListener("click", () => {
        Changerequirement(index, LastElement.ReqElement);
        LastElement.ReqElement = index;
    })
})
requirementButtons.forEach((elem, index) => {
    elem.addEventListener("click", () => {
        Changerequirement(index, LastElement.ReqElement);
        LastElement.ReqElement = index;
    })
})
requirementPrev.addEventListener("click", () => {
    if (LastElement.ReqElement - 1 >= 0) {
        LastElement.ReqElement = --LastElement.ReqElement
        Changerequirement(LastElement.ReqElement, LastElement.ReqElement + 1);
    }
})
requirementNext.addEventListener("click", () => {
    if (LastElement.ReqElement + 1 <= requirementItem.length - 1) {
        LastElement.ReqElement = ++LastElement.ReqElement
        Changerequirement(LastElement.ReqElement, LastElement.ReqElement - 1);
    }
})
